...
export async function generateMetadata({ params }: { params: Promise<{ genre: string }> }): Promise<Metadata> {
  ...
  return {
    title: `${genreName} おすすめAV・動画レビューまとめ【FANZA厳選】`,
    description,
    keywords: [
      `${genreName} AV`,
      `${genreName} おすすめ`,
    ...
    openGraph: {
      title: `${genreName} おすすめAV・動画レビューまとめ`,
      description,
...
export default async function GenrePage({ params }: { params: Promise<{ genre: string }> }) {
  ...
  // 関連ジャンルを集計
  const relatedGenres = Array.from(
    new Set(genrePosts.flatMap(p => (p.genres || []).filter(g => g !== genreName)))
  ).slice(0, 8);
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": `${genreName} レビューまとめ`, "item": `https://haitoku.pages.dev/genre/${genre}` }
    ]
  };
  ...
  const faqSchema = {
    ...
    "mainEntity": [
      {
        "@type": "Question",
        "name": `${genreName}ジャンルのAVはどこで見られますか？`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `${genreName}ジャンルのAVはFANZA（DMM）で配信されています。当サイトでは${genrePosts.length}作品のレビューを掲載しています。`
        }
      },
      {
        "@type": "Question",
        "name": `${genreName}のおすすめ作品は？`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `当サイトでレビューしている${genreName}の人気作品は「${genrePosts[0]?.title || ""}」などです。各作品ページで詳しいレビュー・感想をご確認ください。`
        }
      },
      {
        "@type": "Question",
        "name": `${genreName}に出演している女優は？`,
        "acceptedAnswer": {
          "@type": "Answer",
          "text": `当サイトのレビューに登場する${genreName}ジャンルの女優は「${relatedActresses.slice(0, 5).join("・")}」などです。`
        }
      }
    ]
  };
    ...
    <>
      ...
      <div className="space-y-8 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 justify-between">
            <div className="space-y-3">
              ...
              <p className="text-slate-400 text-sm leading-relaxed max-w-lg">
                {genreName}ジャンルのFANZA人気AV作品を厳選レビュー。当サイトでは
                <strong className="text-slate-200">{genrePosts.length}作品</strong>を掲載しています。
              </p>
            </div>
        ...
        <section className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          <h2 className="text-sm font-extrabold text-slate-800">{genreName} に関するよくある質問</h2>
          <div className="space-y-4 divide-y divide-slate-100">
            <div className="pt-4 first:pt-0 space-y-1.5">
              <p className="text-sm font-bold text-slate-700">Q. {genreName}ジャンルのAVはどこで見られますか？</p>
              <p className="text-xs text-slate-500 leading-relaxed">A. {genreName}ジャンルのAVはFANZA（DMM）で配信されています。当サイトでは{genrePosts.length}作品のレビューを掲載しています。</p>
            </div>
            <div className="pt-4 space-y-1.5">
              <p className="text-sm font-bold text-slate-700">Q. {genreName}のおすすめ作品は？</p>
              <p className="text-xs text-slate-500 leading-relaxed">A. 「{genrePosts[0]?.title || ""}」などがおすすめです。各作品ページで詳しいレビュー・感想をご確認ください。</p>
            </div>
          </div>
        </section>

        <section className="text-center py-6">
          <a href={`https://al.fanza.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fsearch%2F-%2F%3Fsearchstr%3D${encodeURIComponent(genreName)}%2F&af_id=onchan555-003`}
